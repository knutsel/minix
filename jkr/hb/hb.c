/* From http://www.gossamer-threads.com/lists/linux/kernel/238937 */
/* Test groups of X processes spraying to X receivers */
#include <stdio.h>
#include <string.h>
#include <errno.h>
#include <sys/types.h>
...
/* waste an bit of time spinning */
static void spin(int loops) {
	/*
	   struct timeval stim, tim;		
	   gettimeofday(&stim, NULL);
	   double until = stim.tv_usec + usecs;
	   gettimeofday(&tim, NULL);
	   int j = 0;
	   while(tim.tv_usec < until && stim.tv_sec == tim.tv_sec) {
	   gettimeofday(&tim, NULL);
	   j++;
	   }
	 */
	...
	printf("%d.%03d %f ", (int)diff.tv_sec, diff.tv_usec/1000, dd);
	fflush(stdout);
...
again:
		...
		if (done < DATASIZE)
			goto again;
		/* JvD Add some artifical delay/CPU usage */
		spin(SPIN_TIME);
	}
}
...
int main(int argc, char *argv[])
{
	...
	/* Print time... */
	timersub(&stop, &start, &diff);
	printf("\nTime: %d.%03d\n", (int)diff.tv_sec, diff.tv_usec/1000);
	exit(0);
}
